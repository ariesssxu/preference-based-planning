...
def cook_a_frozen_pie(scene_model, headless=False):
    ...
    pie = og.sim.scene.object_registry("name", "cheese_tart_188")
    ...
    video_recorder.set(camera=og.sim.viewer_camera, robot = robot, save_path=f"{og.root_path}/../../images/cook_a_frozen_pie", name="cook_a_frozen_pie", trav_map_img=trav_map_img)
    ...
    for i in range(5):
        ...
        og.sim.step()
    pick_A(pie, robot)
    close_A_with_B(electric_refrigerator, pie, robot)
...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    cfg["task"]["activity_name"] = "cook_a_frozen_pie"
    ...
    cook_a_frozen_pie(cfg["scene"]["scene_model"])
