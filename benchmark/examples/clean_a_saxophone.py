...
def clean_a_saxophone(scene_model, headless=False):
    ...
    rag = og.sim.scene.object_registry("name", "rag_184")
    sax = og.sim.scene.object_registry("name", "saxophone_183")
    scrub_brush = og.sim.scene.object_registry("name", "scrub_brush_185")
    ...
    video_recorder.set(camera=og.sim.viewer_camera, robot = robot, save_path=f"{og.root_path}/../../images/clean_a_saxophones", name="clean_a_saxophone", trav_map_img=trav_map_img)
    ...
    position = scrub_brush.get_position() + np.array([1.2, 0.0, 0.0])
    go_P(position, robot, trav_map, trav_map_img)

    position = sax.get_position() + np.array([0.05, - 0.02, 0.05])
    scrub_brush.set_position(position)
    for i in range(50):
        sax.keep_still()
        video_recorder.get_video()
        og.sim.step()

    position = sax.get_position() + np.array([0.05, 0.03, 0.1])
    scrub_brush.set_position(position)
    for i in range(50):
        sax.keep_still()
        video_recorder.get_video()
    ...
    position = sax.get_position() + np.array([ - 0.08, 0.03, 0.1])
    scrub_brush.set_position(position)
    for i in range(50):
        sax.keep_still()
        video_recorder.get_video()
    ...
    position = sax.get_position() + np.array([ - 0.12, - 0.05, 0.1])
    scrub_brush.set_position(position)
    for i in range(50):
        sax.keep_still()
        video_recorder.get_video()
...
def main(random_selection=False, headless=False, short_exec=False):
    ...
    cfg["task"]["activity_name"] = "clean_a_saxophone"
    ...
    clean_a_saxophone(cfg["scene"]["scene_model"])
